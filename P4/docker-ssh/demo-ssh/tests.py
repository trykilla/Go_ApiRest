...
class APITests(unittest.TestCase):
    ...
    def test_get_doc(self):
        ...
        if response.status_code == NOT_FOUND_STATUS_CODE:
            ...
            self.assertIn(response.json()[
                          "error"], "Wrong id for this user, doc not found or removed.")
    ...
    def test_put_doc(self):
        header = introduce_access_token(self.access_token)
        url = f"{self.base_url}/test_user/test_doc"
        data = {"doc_content": "put_test_content"}
...
if __name__ == '__main__':
    ...
    suite.addTest(APITests("test_put_doc"))
    suite.addTest(APITests("test_delete_doc"))

    result = unittest.TestResult()

    pbar = tqdm(total=len(suite._tests), desc="Tests",
    ...
    print(
        f"\n\n{Fore.GREEN}Success tests/Total tests: {passed_tests}/{total_tests}{Fore.RESET}")

    # Imprimir detalles de pruebas que fallaron
    if result.errors or result.failures:
        print(f"\n{Fore.RED}Failed tests:{Fore.RESET}")
        for failure in result.errors:
            print(f"\n{Fore.RED}Error in test: {failure[0]}{Fore.RESET}")
            print(f"Error info: {failure[1]}")
